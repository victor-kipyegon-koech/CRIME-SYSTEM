...
export const reportPriorityEnum = pgEnum("reportPriority", ["low", "medium", "high", "critical"]);

export const evidenceTypeEnum = pgEnum("evidenceType", ["image", "video", "audio", "document", "other"]);
...
export const crimeReportTable = pgTable("crimeReportTable", {
  ...
  categoryId: integer("categoryId").notNull().references(() => crimeCategoryTable.categoryId, { onDelete: "restrict" }),
  ...
  assignedOfficerId: integer("assignedOfficerId").references(() => userTable.userId, { onDelete: "set null" }),
...
export const evidenceTable = pgTable("evidenceTable", {
  ...
  uploadedBy: integer("uploadedBy").references(() => userTable.userId, { onDelete: "set null" }),
  createdAt: timestamp("createdAt").defaultNow(),
});
...
export const caseAssignmentTable = pgTable("caseAssignmentTable", {
  ...
  reportId: integer("reportId").notNull().references(() => crimeReportTable.reportId, { onDelete: "cascade" }),
  officerId: integer("officerId").notNull().references(() => userTable.userId, { onDelete: "cascade" }),
  assignedBy: integer("assignedBy").references(() => userTable.userId, { onDelete: "set null" }),
...
export const reportUpdateTable = pgTable("reportUpdateTable", {
  ...
  reportId: integer("reportId").notNull().references(() => crimeReportTable.reportId, { onDelete: "cascade" }),
  updatedBy: integer("updatedBy").references(() => userTable.userId, { onDelete: "set null" }),
...
export const notificationTable = pgTable("notificationTable", {
  ...
  userId: integer("userId").notNull().references(() => userTable.userId, { onDelete: "cascade" }),
  reportId: integer("reportId").references(() => crimeReportTable.reportId, { onDelete: "cascade" }),
...
export const crimeReportRelations = relations(crimeReportTable, ({ one, many }) => ({
  reporter: one(userTable, { fields: [crimeReportTable.reporterId], references: [userTable.userId] }),
  assignedOfficer: one(userTable, { fields: [crimeReportTable.assignedOfficerId], references: [userTable.userId] }),
  category: one(crimeCategoryTable, { fields: [crimeReportTable.categoryId], references: [crimeCategoryTable.categoryId] }),
...
export const evidenceRelations = relations(evidenceTable, ({ one }) => ({
  report: one(crimeReportTable, { fields: [evidenceTable.reportId], references: [crimeReportTable.reportId] }),
  uploader: one(userTable, { fields: [evidenceTable.uploadedBy], references: [userTable.userId] }),
}));

export const caseAssignmentRelations = relations(caseAssignmentTable, ({ one }) => ({
  report: one(crimeReportTable, { fields: [caseAssignmentTable.reportId], references: [crimeReportTable.reportId] }),
  officer: one(userTable, { fields: [caseAssignmentTable.officerId], references: [userTable.userId] }),
  assignedByUser: one(userTable, { fields: [caseAssignmentTable.assignedBy], references: [userTable.userId] }),
}));

export const reportUpdateRelations = relations(reportUpdateTable, ({ one }) => ({
  report: one(crimeReportTable, { fields: [reportUpdateTable.reportId], references: [crimeReportTable.reportId] }),
  updatedByUser: one(userTable, { fields: [reportUpdateTable.updatedBy], references: [userTable.userId] }),
}));

export const notificationRelations = relations(notificationTable, ({ one }) => ({
  user: one(userTable, { fields: [notificationTable.userId], references: [userTable.userId] }),
